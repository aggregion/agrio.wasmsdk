...
namespace agrio {
   ...
   struct margin_position {
      ...
      AGRLIB_SERIALIZE( margin_position, (owner)(borrowed)(collateral)(call_price) )
   };

   typedef agrio::multi_index<N(margins), margin_position,
           indexed_by<N(callprice), agrio::const_mem_fun<margin_position, uint64_t, &margin_position::get_call> >
   > margins;
   ...
   struct loan_position {
      ...
      AGRLIB_SERIALIZE( loan_position, (owner)(interest_shares) )
   };

   typedef agrio::multi_index<N(loans), loan_position> loans;
