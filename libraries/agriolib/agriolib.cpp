...
#include "contracts/agrio/system.hpp"
#include "contracts/agrio/privileged.hpp"
...
namespace agrio {
   ...
   void get_blockchain_parameters(agrio::blockchain_parameters& params) {
      ...
      agrio::check( size <= sizeof(buf), "buffer is too small" );
   ...
   std::optional<uint64_t> set_proposed_producers( const std::vector<producer_key>& prods ) {
      auto packed_prods = agrio::pack( prods );
