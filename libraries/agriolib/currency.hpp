...
namespace agrio {
    ...
    *  @deprecated This class is deprecated in favor of agrio.token in Dawn 3.0
    */
   class currency {
      public:
         ...
         struct create {
           ...
           // array<char,32>         issuer_agreement_hash;
            ...
            AGRLIB_SERIALIZE( create, (issuer)(maximum_supply)(issuer_can_freeze)(issuer_can_recall)(issuer_can_whitelist) )
         };
         ...
         struct transfer
         {
            ...
            AGRLIB_SERIALIZE( transfer, (from)(to)(quantity)(memo) )
         };
         ...
         struct issue {
            ...
            AGRLIB_SERIALIZE( issue, (to)(quantity)(memo) )
         };
         ...
         struct fee_schedule {
            ...
            AGRLIB_SERIALIZE( fee_schedule, (fee_per_length) )
         };
         ...
         struct account {
            ...
            AGRLIB_SERIALIZE( account, (balance)(frozen)(whitelist) )
         };
         ...
         struct currency_stats {
            ...
            AGRLIB_SERIALIZE( currency_stats, (supply)(max_supply)(issuer)(can_freeze)(can_recall)(can_whitelist)(is_frozen)(enforce_whitelist) )
         };
         ...
         bool apply( account_name contract, action_name act ) {
          ...
          void issue_currency( const issue& i ) {
             ...
             statstable.modify( st, 0, [&]( auto& s ) {
                ...
                agrio_assert( s.supply.amount >= 0, "underflow" );
             });
          ...
          void on( const issue& i ) {
             ...
             agrio_assert( i.quantity.is_valid(), "invalid quantity" );
             agrio_assert( i.quantity.amount > 0, "must issue positive quantity" );
          ...
          void on( const transfer& t ) {
             ...
             agrio_assert( t.quantity.is_valid(), "invalid quantity" );
             agrio_assert( t.quantity.amount > 0, "must transfer positive quantity" );
      ...
      private:
          void sub_balance( account_name owner, asset value, const currency_stats& st ) {
             ...
             if( has_auth( owner ) ) {
                agrio_assert( !st.can_freeze || !from.frozen, "account is frozen by issuer" );
                agrio_assert( !st.can_freeze || !st.is_frozen, "all transfers are frozen by issuer" );
                agrio_assert( !st.enforce_whitelist || from.whitelist, "account is not white listed" );
             } else if( has_auth( st.issuer ) ) {
                agrio_assert( st.can_recall, "issuer may not recall token" );
             } else {
                agrio_assert( false, "insufficient authority" );
             }
          ...
          void add_balance( account_name owner, asset value, const currency_stats& st, account_name ram_payer )
          {
             ...
             if( to == to_acnts.end() ) {
                agrio_assert( !st.enforce_whitelist, "can only transfer to white listed accounts" );
                to_acnts.emplace( ram_payer, [&]( auto& a ){
                  a.balance = value;
                });
             } else {
                agrio_assert( !st.enforce_whitelist || to->whitelist, "receiver requires whitelist by issuer" );
