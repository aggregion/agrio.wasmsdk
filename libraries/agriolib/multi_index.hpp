 ...
 *      AGRLIB_SERIALIZE( record, (primary)(secondary_1)(secondary_2)(secondary_3)(secondary_4)(secondary_5) )
 *    };
 *    public:
 *      mycontract( account_name self ):contract(self){}
 *      void myaction() {
...
class multi_index
{
   ...
   public:
       ...
       *
       *  Notes
       *  The `agrio::multi_index` template has template parameters `<uint64_t TableName, typename T, typename... Indices>`, where:
       ...
       *       AGRLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef agrio::multi_index< N(address), address > address_index;
       *      void myaction() {
       ...
       *        agrio_assert(addresses.get_code() == N(dan), "Codes don't match.");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses(N(dan), N(dan)); // code, scope
       *        agrio_assert(addresses.get_code() == N(dan), "Scopes don't match");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        agrio_assert(itr->account_name == N(dan), "Incorrect Last Record ");
       *        itr++;
       *        agrio_assert(itr->account_name == N(brendan), "Incorrect Second Last Record");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        agrio_assert(itr->account_name == N(brendan), "Incorrect First Record ");
       *        itr--;
       *        agrio_assert(itr->account_name == N(dan), "Incorrect Second Record");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *       AGRLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state)(zip) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef agrio::multi_index< N(address), address, indexed_by< N(zip), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self);  // code, scope
       *        // add to table, first argument is account to bill for storage
       *        addresses.emplace(payer, [&](auto& address) {
       ...
       *        agrio_assert(itr->account_name == N(brendan), "Incorrect First Lower Bound Record ");
       *        itr++;
       *        agrio_assert(itr->account_name == N(dan), "Incorrect Second Lower Bound Record");
       *        itr++;
       *        agrio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        agrio_assert(itr->account_name == N(dan), "Incorrect First Upper Bound Record ");
       *        itr++;
       *        agrio_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       ...
       *       AGRLIB_SERIALIZE( address, (key)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef agrio::multi_index< N(address), address > address_index;
       ...
       *        agrio_assert(itr->account_name == N(dan), "Incorrect Record ");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         agrio_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *          address.state = "CA";
       *        });
       *        agrio_assert(itr->city == "San Luis Obispo", "Address not modified");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         agrio_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *        agrio_assert(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        agrio_assert(itr != addresses.end(), "Address not erased properly");
       *      }
       *  }
       *  AGRIO_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         agrio_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
