...
namespace agrio {   
   struct dsmalloc {
      ...
      char* operator()(size_t sz, uint8_t align_amt=8) {
         ...
         agrio::check(GROW_MEMORY(pages_to_alloc) != -1, "failed to allocate pages");  
         return ret;
      }
...
void* malloc(size_t size) {
   void* ret = agrio::_dsmalloc(size);
   return ret;
}

void* memset(void*,int,size_t);
void* calloc(size_t count, size_t size) {
   if (void* ptr = agrio::_dsmalloc(count*size)) {
