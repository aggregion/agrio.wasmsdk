...
#warning "<agrio/native/tester.hpp> is deprecated use <agrio/tester.hpp>"
...
inline bool expect_assert(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   if (ret == 0) {
      ...
      if (!check)
         agrio_assert(false, std::string("error : expect_assert, no assert {"+li+"}").c_str());
   ...
   silence_output(false);
   if (!check)
      agrio_assert(passed, std::string("error : expect_assert, wrong assert {"+li+"}").c_str());
   if (!passed)
...
inline bool expect_print(bool check, const std::string& li, Pred&& pred, F&& func, Args... args) {
   ...
   silence_output(false);
   if (!check)
      agrio_assert(passed, std::string("error : wrong print message {"+li+"}").c_str());
   if (!passed)
...
#define REQUIRE_EQUAL(X, Y) \
   agrio_assert(X == Y, std::string(std::string("REQUIRE_EQUAL failed (")+#X+" != "+#Y+") {"+__FILE__+":"+std::to_string(__LINE__)+"}").c_str());
