	...
	if [ "${OS_VER}" -lt 7 ]; then
		printf "\\n\\tYou must be running Centos 7 or higher to install AGRIO.\\n"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi
	...
	if [ -z "${SCL}" ]; then
		printf "\\n\\tThe Centos Software Collections Repository, devtoolset-7 and Python3 are required to install AGRIO.\\n"
