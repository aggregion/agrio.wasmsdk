...
#include <agrio/gen.hpp>

#include <agrio/utils.hpp>
#include <agrio/whereami/whereami.hpp>
#include <agrio/abi.hpp>
...
namespace agrio { namespace cdt {
   ...
   struct codegen_exception : public std::exception {
      virtual const char* what() const throw() {
         return "agrio.codegen fatal error";
      }
   };
   ...
   // remove after v1.7.0
   bool has_agriolib = false;

   class agrio_ppcallbacks : public PPCallbacks {
      public:
         agrio_ppcallbacks(SourceManager& sm, std::string file) : sources(sm), fn(file) {}
      protected:
         virtual void InclusionDirective(
            ...
            if ( file_name.find("agriolib") != StringRef::npos )
               has_agriolib = true;
         }
   ...
   class agrio_codegen_visitor : public RecursiveASTVisitor<agrio_codegen_visitor>, public generation_utils {
      private:
      ...
      public:
         ...
         explicit agrio_codegen_visitor(CompilerInstance *CI)
         ...
         bool is_datastream(const QualType& qt) {
            ...
            auto ds_re    = std::regex("(((class agrio::)?datastream<[a-zA-Z]+[a-zA-Z0-9]*.*>)|(DataStream)) &");
         ...
         */
         ...
         void create_dispatch(const std::string& attr, const std::string& func_name, F&& get_str, CXXMethodDecl* decl) {
            ...
            if (cg.is_agrio_contract(decl, cg.contract_name)) {
               if (has_agriolib) {
                  ss << "\n\n#include <agriolib/datastream.hpp>\n";
                  ss << "#include <agriolib/name.hpp>\n";
               } else {
                  ss << "\n\n#include <agrio/datastream.hpp>\n";
                  ss << "#include <agrio/name.hpp>\n";
               }
               ss << "extern \"C\" {\n";
               ...
               ss << "}\n";
               ss << "agrio::datastream<const char*> ds{(char*)buff, as};\n";
               int i=0;
               ...
               ss << decl->getParent()->getQualifiedNameAsString() << "{agrio::name{r},agrio::name{c},ds}." << decl->getNameAsString() << "(";
         ...
         void create_action_dispatch(CXXMethodDecl* decl) {
            ...
            create_dispatch("agrio_wasm_action", "__agrio_action_", func, decl);
         }

         void create_notify_dispatch(CXXMethodDecl* decl) {
            auto func = [](CXXMethodDecl* d) { return generation_utils::get_notify_pair(d); };
            create_dispatch("agrio_wasm_notify", "__agrio_notify_", func, decl);
         }
         ...
         virtual bool VisitCXXMethodDecl(CXXMethodDecl* decl) {
            ...
            if (decl->isAgrioAction()) {
               name = generation_utils::get_action_name(decl);
               validate_name(name, [&]() {emitError(*ci, decl->getLocation(), "action not a valid agrio name");});
      ...
      class agrio_codegen_consumer : public ASTConsumer {
      private:
         agrio_codegen_visitor *visitor;
         std::string main_file;
         CompilerInstance* ci;

      public:
         explicit agrio_codegen_consumer(CompilerInstance *CI, std::string file)
            : visitor(new agrio_codegen_visitor(CI)), main_file(file), ci(CI) { }
         ...
         virtual void HandleTranslationUnit(ASTContext &Context) {
            ...
            if (main_fe) {
               ...
               try {
                  ...
                  std::stringstream ss;
                  ss << "extern \"C\" {\n";
                  ss << "void agrio_assert_code(uint32_t, uint64_t);";
                  ss << "\t__attribute__((weak, agrio_wasm_entry, agrio_wasm_abi(";
                  std::string abi = cg.abi;
                  ss << "\"" << _quoted(abi) << "\"";
                  ss << ")))\n";
                  ss << "\tvoid __insert_agrio_abi(unsigned long long r, unsigned long long c, unsigned long long a){";
                  ss << "agrio_assert_code(false, 1);";
                  ss << "}\n";
                  ss << "}";
      ...
      class agrio_codegen_frontend_action : public ASTFrontendAction {
      public:
         virtual std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI, StringRef file) {
            CI.getPreprocessor().addPPCallbacks(_make_unique<agrio_ppcallbacks>(CI.getSourceManager(), file.str()));
            return _make_unique<agrio_codegen_consumer>(&CI, file);
         }
   };
